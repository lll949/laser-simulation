...
%% ==================== 增益介质设置 ====================
gain_positions =[20,60]*mm;
gain_thickness = [0.55,1.45]*mm;
...
pump_field =  sqrt(pu) .* ring_amplitude.*global_vortex_phase.* (cos(5*phi_global));
%% ==================== 主模拟循环：GPU优化版 ====================
N_iterations = 60;
...
% 
% subplot(2,2,3);
% plot(1:N_iterations, mode_fraction_pos, 'r-', 'LineWidth', 1.5);
% hold on;
% plot(1:N_iterations,mode_fraction_neg, 'b-', 'LineWidth', 1.5);
% xlabel('迭代次数');
% ylabel('模式占比');
% title('模式占比演化');
% legend('LG+1', 'LG-1');
...
% % for frame = frames_to_save
% %     clf(fig);   % 清空图形窗口
%     imagesc(fftshift(var.x_)/mm, fftshift(var.y_)/mm, fftshift(abs(Pump_field_current).^2));
%     axis equal tight;
%     axis off;
%     colormap jet;
%     filename = 'pump_intensity3001.png';   % 或者 sprintf('frame_%d.png', frame_index)
% exportgraphics(fig, filename, 'Resolution', 2000);
% end
% 
% close(fig);   % 关闭图形窗口
% % 假设 mode_fraction_pos 和 mode_fraction_neg 是列向量或行向量
% T = table((1:N_iterations)', mode_fraction_pos', mode_fraction_neg', ...
%     'VariableNames', {'Iteration', 'ModeFraction_Pos', 'ModeFraction_Neg'});
% writetable(T, 'mode_fraction_data.xlsx');
% 取第一块增益介质、第一个子段的反转粒子数（示例）
% gain_idx = 4;      % 第几块增益介质
% segment_idx = 5;   % 该介质内的第几个子段
% 
% % 从 GPU 取回数据并转换到 CPU
% N_inv_show = gather(N_inv_cumulative(:, :, gain_idx, segment_idx));
% % 
% figure;
% imagesc(fftshift(var.x_)/mm, fftshift(var.y_)/mm, fftshift(N_inv_show));
% axis equal tight;
% axis off;
% colormap(jet);

%    frame = getframe(gca);
% imwrite(frame.cdata, 'output.tif');
%     figure;
% 
% % 存储所有数据用于统一坐标轴范围

iterations_to_show = [3, 15, 60];
...
% 导出为 CSV 文件
writetable(all_data, '模式占比数据1002.csv');

% 导出为 Excel 文件
writetable(all_data, '模式占比数据1002.xlsx');

% 可选：显示导出成功信息
fprintf('数据已导出到当前目录下的 模式占比数据.csv 和 模式占比数据.xlsx\n');
